...
- **An edition of a discipline** is an edition of the sequence where a discipline of that
  name has a ranked result (`ResultStanding.ranking` above 0), so revealed, scored and
...
- `test_badges.py`, `TestDisciplines`: earned at the second edition, one edition is not
  ...
  discipline and hidden or uncontested ones do not count, a shared 1st place is a win,
  two disciplines of one name are two events, one badge per discipline. `MASTER` is
  among the `DISCIPLINE_CODES` that a hidden or hand-ranked edition never gives. The
  discipline helpers moved into a `DisciplineWorld` mixin for reuse.
