...
> **Executed 2026-09-23.** The code blocks below are the plan as written. After #90 (places per discipline) merged, `_discipline_results` reads each edition's `Standings.disciplines_of` data instead of its own `TeamResult` query (`DisciplineStanding` gained `result_type` and `points`), so `BADGES_QUERIES` is 4 + 3 per sequence edition. Review rounds changed several details during execution: `reign` breaks on a table after an edition where no participation counts (`_tables` also yields whether its edition counts), `MINDS` is derived from `FAMILIES` (the Athena disciplines plus Blindtest) instead of a hand list, `BadgeAdmin` also filters on `is_active`, `badgeTier()` in `badges.js` clamps the tier so the metal, the pips and « Niveau n » agree, the profile computes the detail parts once and hides the ` · ` separators from screen readers, the French rules say « épreuve » and several rules were reworded, the tested text shape is `Clean sweep ×2 · 2023 · 2026 Win three disciplines or more in one edition`, and the all-rounder, brains-and-brawn, hermes, hades and clean-sweep glyphs were redrawn. The final review added: `golden-whistle` counts a game only when its referee team is neither of the two playing (the schedulers leave a playing team there as a placeholder); `refresh()` reads only the rows of active editions, so an inactive edition keeps its badges; of the duplicates of a key, `refresh()` keeps an inactive row first, so a revocation survives, else the lowest id; computed rows cannot be deleted in the admin (`has_delete_permission` on the Badge admin's own pages, so deleting a user or an edition still cascades, and `delete_selected` and `delete_queryset` keep only manual rows); the Edition action requires the change permission and its message gives the refresh time in Paris, which the Badge changelist also shows; and a failed refresh after a committed `import_edition` ends in a `CommandError` that says so. The spec, CLAUDE.md and the code are the reference; this plan is the build record.
